...
def test_general_hough(gh, reference_image, query, n):
    '''
    Uses a GH closure to detect shapes in an image and create nice output
    '''
    query_image = imread(query, mode="F")
...
def general_hough(cross="s.png", cross_test="s_test.png", n=5):
    reference_image = imread(cross, mode="F")
    detect_s = general_hough_closure(reference_image)
    return test_general_hough(detect_s, reference_image, cross_test, n)
